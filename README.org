...
#+BEGIN_SRC emacs-lisp
  (use-package base16-theme
    ...
    (set-face-attribute 'line-number-current-line nil :foreground (plist-get base16-material-colors :base01))
    (set-face-attribute 'line-number-current-line nil :background (plist-get base16-material-colors :base04)))
...
(setq org-sport-file "~/org/sport.org")
#+END_SRC
*** Capture
...
#+end_src
*** Capture Templates
Save todos in the default notes file showing list of headings as possible targets. Sport file is organized in weekly format.
...
(setq org-capture-templates
      '(("t" "Todo" entry (file+function org-default-notes-file refile-heading)
	 "* TODO %?\nSCHEDULED: %(org-insert-time-stamp nil)")
	("s" "Sport")
	("sw" "Weight" plain (file+datetree+prompt org-sport-file "Timeline") "%^{WEIGHT}p" :tree-type week)
	("sr" "Running" entry (file+datetree+prompt org-sport-file "Timeline")
	 "* Running\n- distance: %^{Distance} km\n- time: %^{Time} min\n- notes: %^{Notes}" :tree-type week)
	("sg" "Gym" entry (file+datetree+prompt org-sport-file "Timeline")
	 "* Gym\n%?" :tree-type week)))
#+end_src
* Git
