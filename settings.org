...
;; JUST FOR FUCKING CLOJURE-MODE
;; TODO: remove this ASAP.
;; (add-to-list 'package-archives ' ("melpa" . "http://melpa.org/packages/") t)
(package-initialize)
#+END_SRC

I have some local packages that I'm trying out, when I finish creating them, they will go to MELPA and there will be no need for local repo.
#+BEGIN_SRC emacs-lisp
(add-to-list 'load-path "~/.emacs.d/djole/")
#+END_SRC
...
#+END_SRC
*** Code confirmation
g never accidentally type ~C-c C-c~ so there is no need for confirmation.
#+BEGIN_SRC emacs-lisp
...
*** Baby
##+BEGIN_SRC emacs-lisp
(add-to-list 'load-path "./djole")
(setq trainlog-file "~/org/beba1.org") 
(setq baby-file "~/org/beba2.org") 
(setq org-capture-templates '()) ;; RESET TEMPLATES

;;(trainlog-create-training-template :glavno '("prva opcija" "druga opcija" "treca opcija"))

(defun djole-cap--create-daily-entry(file keyword title tags)
  (add-to-list 'org-capture-templates `(,(format "%s%s" (djole-prefix keyword) (djole-prefix title))
					,(format "%s" title) entry (file+datetree+prompt ,file)
					,(format "* %s %s%%(djole-cap--force-tag):" title keyword) :time-prompt t ,:tags ,tags)))

;(defun djole-title(input)
(defun djole-cap-template (file key values)
  (add-to-list 'org-capture-templates `(,(djole-prefix key)
					,(substring (capitalize (symbol-name key) 1)))
  (while values
    (let ((value (pop values)))
      (cond ((consp value) (djole-cap--create-daily-entry file key (car value) (cdr value)))
	    ((stringp value) (djole-cap--create-daily-entry file key value nil))))))

(djole-cap-template baby-file :baby '("prvi" "drugi" "treci"))

##+END_SRC
*** Gym

(defun add-training-template (shortcut title options)
  (add-to-list 'org-capture-templates
	       `(,shortcut ,(format "%s Training" title) entry (file+olp+datetree org-training-file)
			   ,(format "* %%^{Training|%s|%s} :%s:" title (mapconcat 'identity options "|") (downcase title))
			   :time-prompt t :tree-type week)))
(defun add-exercise-template (shortcut title pattern)
  (add-to-list 'org-capture-templates `(,shortcut ,title entry (file+function org-training-file org-gym-last-session)
						  ,(concat "* %(org-gym-exercises) " pattern))))



(add-training-template "g" "Gym" '("Training" "Gym" "5/3/1-Bench" "5/3/1-Squat" "5/3/1-Press" "5/3/1-Deadlift" "Push" "Pull" "Legs"))
(add-training-template "c" "Cardio" '("Training" "Cardio" "Running" "Bike"))
(add-exercise-template "e" "Exercise" "\nsets: %^{sets}\t reps: %^{reps}\t weight: %^{weight} kg\n%?")

Treci pokusaj, izgleda dobro 
...
(add-training-template "t" "Training" '("Gym" "Gym" "5/3/1-Bench" "5/3/1-Squat" "5/3/1-Press" "5/3/1-Deadlift" "Push" "Pull" "Legs"))
...
#+END_SRC
** Lisp
Run whole buffer on easy shortcut.
#+BEGIN_SRC emacs-lisp
(define-key emacs-lisp-mode-map (kbd "C-c q") 'eval-buffer)
#+END_SRC

Smartparens is one possible options for lisp.
#+BEGIN_SRC emacs-lisp :tangle no
...
* Templejti
   #+BEGIN_SRC emacs-lisp
   ;; (setq gymorg-file "~/org/training.org")
   ;; (setq org-capture-templates '())
   ;; (setq gymorg-training-options '((gym "5/3/1" "PPL" "Conditioning")
   ;;                                 (cardio "Running" "Bike" "Rowing")
   ;;                                 (sports "Football" "Archery")))


   ;; (add-to-list 'org-capture-templates `("g" "Gym" entry (file+olp+datetree gymorg-file)
   ;;                                       ,(format "* %%(gymorg--tag-picker '(\"Type of training: \" 5/3/1 PPL Conditioning)) %%?") :time-prompt t :tree-type week))

   ;; (defun gymorg-pick-training ()
   ;;   (interactive)
   ;;   (let* ((pick (intern (completing-read "Type of training: " gymorg-training-options nil t)))
   ;;          (type (completing-read "Activity: " (cdr (assoc pick gymorg-training-options)))))
   ;;     (format "%s :%s:" type pick)))


   ;; (add-to-list 'org-capture-templates `("t" "Training" entry (file+olp+datetree gymorg-file)
   ;;                                       ,(format "* %%(gymorg-pick-training)%%?") :time-prompt t :tree-type week))

   ;; (defun add-exercise-template (shortcut title tags pattern)
   ;;   (setq local_tags tags)
   ;;   (add-to-list 'org-capture-templates `(,shortcut ,title entry (file+function gymorg-file gymorg--last-session)
   ;;                                                   ,(format "* %%(gymorg--exercises) %%(gymorg--tag-list local_tags)\n%s" pattern))))

   ;; (add-exercise-template "g" "Gym Exercise"
   ;;                        '(("Equipment: " barbell dumbell ez_bar kettlebell bodyweight machine cable medicine_ball) 
   ;;                          ("Type: " dropset superset))
   ;;                        "sets: %^{sets}\t reps: %^{reps}\t weight: %^{weight} kg\n%?")

   ;; (add-to-list 'org-capture-templates '("c" "Cardio Exercise" entry (file+function gymorg-file gymorg--last-session)
