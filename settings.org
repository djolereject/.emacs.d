...
#+END_SRC
** use-package
I stated everything I think of ~use-package~ in intro, so I won't explain further why it's my preferred installer. This setup could obviously work with some other macro, but I'm not sure if some discrepancies could arise.
...
#+END_SRC
*** Clear
It might be personal quirk but most frequent use of ~C-l~ for me is to move cursor position to top of the screen, so I usually type ~C-l C-l~. Whenever something is repeating, aim for simplification.
...
#+END_SRC
* Theme
Picking theme is personal for everybody. If you don't like my choice, explore some resources out there and pick one that suits you. For now, I opted for ~github~ theme from ~base16~. This will change often if history is any indicator but it's clean and I wanted to try light theme for once. There are lot of repositories out there so you shouldn't limit yourself to ~base16~, [[https://belak.github.io/base16-emacs/][but they do have some variety.]]
...
#+END_SRC
* Git
Version control is important part of Emacs ever since [[https://github.com/magit/magit][Magit]] entered the scene showing factual difference between "porcelain" and "plumbing". After spending some time getting used to it, efficiency raised with ~Magit~ will look like magic to seasoned git user.
...
TODO: This is the section I will come back to!
